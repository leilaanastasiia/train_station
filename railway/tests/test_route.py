...
class TestRoute(BaseSetUp):
    ...
    def test_route_is_not_valid(self):
        with pytest.raises(ValueError, match="Route's start or end must be "
                                            "an object of Station class."):
            Route('Kyiv', 'Zhytomyr')
    ...
    def test_route_get_way(self, routes):
        assert routes[2].get_way() == ['Kyiv', 'Yalta']
